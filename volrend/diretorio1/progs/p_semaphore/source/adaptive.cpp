...
void Ray_Trace(long my_node) {
    ...
    Pre_Shade(my_node);
    sem_wait(&Global->CountLock);
    Global->Counter--;
    sem_post(&Global->CountLock);
...
void Ray_Trace_Non_Adaptively(long my_node) {
    ...
    while (Global->Queue[num_nodes][0] > 0) {
        ...
        sem_wait(&Global->QLock[local_node]);
        work = Global->Queue[local_node][0]++;
        sem_post(&Global->QLock[local_node]);
        ...
        if (my_node == local_node) {
            sem_wait(&Global->QLock[num_nodes]);
            Global->Queue[num_nodes][0]--;
            sem_post(&Global->QLock[num_nodes]);
        }
