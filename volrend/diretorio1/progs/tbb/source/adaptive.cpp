...
void Ray_Trace(long my_node) {
    ...
    Global->TimeBarrier->await();

    Pre_Shade(my_node);
    Global->CountLock.lock();
    Global->Counter--;
    Global->CountLock.unlock();
...
void Ray_Trace_Non_Adaptively(long my_node) {
    ...
    while (Global->Queue[num_nodes][0] > 0) {
        ...
        Global->QLock[local_node].lock();
        work = Global->Queue[local_node][0]++;
        Global->QLock[local_node].unlock();
        ...
        if (my_node == local_node) {
            Global->QLock[num_nodes].lock();
            Global->Queue[num_nodes][0]--;
            Global->QLock[num_nodes].unlock();
        }
