...
#define MAP_ADDRESS(IZ,IY,IX)	 (map_address+((IZ)*map_len[Y]+(IY))*map_len[X]+(IX))
#define MAP(IZ,IY,IX)		     (*MAP_ADDRESS(IZ,IY,IX))

#define GMAG_ADDRESS(IZ,IY,IX)	 (gmag_address+((IZ)*gmag_len[Y]+(IY))*gmag_len[X]+(IX))
#define GMAG(IZ,IY,IX)		     (*GMAG_ADDRESS(IZ,IY,IX))

#define TADDR(IZ,IY,IX)	         (LOOKUP_HSIZE+((IZ)*LOOKUP_PREC+(IY))*2+(IX))

#define NORM_ADDRESS(IZ,IY,IX,D) (norm_address+((IZ)*norm_len[Y]+(IY))*norm_len[X]+(IX))

#define NORM(IZ,IY,IX,D)		 (*NORM_ADDRESS(IZ,IY,IX,D))

#define OPC_ADDRESS(IZ,IY,IX)	 (opc_address+((IZ)*opc_len[Y]+(IY))*opc_len[X]+(IX))
#define OPC(IZ,IY,IX)		     (*OPC_ADDRESS(IZ,IY,IX))

#define VOX_ADDRESS(IZ,IY,IX,D)	 (vox_address+(((IZ)*vox_len[Y]+(IY))*vox_len[X]+(IX))*\
				  2+(D))
