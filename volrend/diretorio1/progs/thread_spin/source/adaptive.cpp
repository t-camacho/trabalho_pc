...
void Ray_Trace_Non_Adaptively(long my_node) {
    ...
    while (Global->Queue[num_nodes][0] > 0) {
        ...
        pthread_spin_lock(&Global->QLock[local_node]);
        work = Global->Queue[local_node][0]++;
        pthread_spin_unlock(&Global->QLock[local_node]);
        ...
        if (my_node == local_node) {
            pthread_spin_lock(&Global->QLock[num_nodes]);
            Global->Queue[num_nodes][0]--;
            pthread_spin_unlock(&Global->QLock[num_nodes]);
        }
