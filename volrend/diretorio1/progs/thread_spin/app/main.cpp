...
void Frame() {
    ...
    pthread_spin_init(&Global->IndexLock, PTHREAD_PROCESS_PRIVATE);
    pthread_spin_init(&Global->CountLock, PTHREAD_PROCESS_PRIVATE);
    ...
    for(long i = 0; i < MAX_NUMPROC+1; i++) {
        pthread_spin_init(&Global->QLock[i], PTHREAD_PROCESS_PRIVATE);
    }
...
void* Render_Loop(void *arg) {
    ...
    pthread_spin_lock(&Global->IndexLock);
    my_node = Global->Index++;
    pthread_spin_unlock(&Global->IndexLock);
